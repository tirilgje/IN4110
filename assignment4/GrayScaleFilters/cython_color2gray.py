...
import numba_color2gray as nbc2g
...
def timing_cython_gray(n):
    
    """
    Calls the grayscale function n times and returns the mean time 
    
    Input: int n - number of interations 
    Returns (int): the mean time after n runs 
    ...
    for i in range(n):
        ...
        cython_color2gray("rain.jpeg", "cython_rain.jpeg")
...
if __name__ == "__main__":
    ...
    mean_t = timing_cython_gray(n)
    
    mean_python = pyc2g.timing_python_gray(n)
    mean_numpy = npc2g.timing_numpy_gray(n)
    mean_numba = nbc2g.timing_numba_gray(n)
    
    compare_python = mean_python/mean_t
    compare_numpy = mean_t/mean_numpy
    compare_numba = mean_t/mean_numba
    
    
    f = open("cython_report_color2gray.txt", "w")
    
    f.write("Timing : cython_color2gray" 
            "\nAverage runtime running cython_color2gray after 3 runs : " + str(round(mean_t, 3)) + " s" 
            "\nAverage runtime running of cython_color2gray is " 
            + str(round(compare_python, 3)) + " times faster than python_color2gray."
            
            "\nAverage runtime for running cython_color2gray is " 
            + str(round(compare_numpy, 3)) + " times slower than numpy_color2gray.py"
            
            "\nAverage runtime for running cython_color2gray is " 
            + str(round(compare_numpy, 3)) + " times slower than numba_color2gray.py"
